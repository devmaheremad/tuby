...
} from "./ChannelDetail.types";
import VideoCard from "./VideoCard";
import HashLoader from "react-spinners/HashLoader";

const ChannelDetail = () => {
  const { id } = useParams();
  const [channelInfo, setChannelInfo] = useState<ChannelDetailInfoType>(
    {} as ChannelDetailInfoType
  );
  const [channelVideos, setChannelVideos] = useState<ChannelDetailVideosType[]>(
    [{} as ChannelDetailVideosType]
  );

  useEffect(() => {
    fetchFromRapidAPI(`channels?part=snippet,statistics&id=${id}`).then(
      (data) => setChannelInfo(data.items[0])
    );
